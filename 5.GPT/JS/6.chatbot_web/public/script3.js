document.addEventListener("DOMContentLoaded", async function() {
    ...
    function addSessionClickEventListeners() {
        const sessionLinks = document.querySelectorAll('.session-link');
        sessionLinks.forEach(link => {
            link.addEventListener('click', async (event) => {
                event.preventDefault();
                const sessionId = link.dataset.sessionId;
                if (sessionId === currentSessionId.textContent) {
                    return; // 이미 선택된 세션이면 아무것도 하지 않음
                }
    ...
    async function showSession(sessionId) {
        ...
        currentSessionId.textContent = data.id; // Update current session ID
        data.conversationHistory.forEach(item => {
            displayMessage(item.content, item.role); // Append messages to   chat container
        }); // Append new messages
        displaySessionInfo(data); // Update session info
    }
